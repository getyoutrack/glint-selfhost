...
Please use this README if you want to deploy Glint on your server with `docker compose`. I'm using a Basic Droplet on Digital Ocean with Ubuntu 23.10, but these instructions can be easily adapted for any Linux distribution.

> [!NOTE]
> Glint is quite resource-heavy, so I recommend using a Droplet with 2 vCPUs and 4GB of RAM. Droplets with less RAM may stop responding or fail.
...
## Clone the `glint-selfhost` repository and configure `nginx`

Next, let's clone the `glint-selfhost` repository and configure the server address. _Please replace **x.y.z.w** with your server's IP address_.
...
## Now we're ready to run Glint

Finally, let's restart `nginx` and run Glint with `docker compose`.
...
When exposing your self-hosted Glint deployment to the internet, it's crucial to implement some security measures to protect your server and data.

1. Do not expose MongoDB, MinIO, and Elastic services to the internet. Glint does not require them to be accessible from the internet.
...
2. Add email address you'll use to send notifications into "SOURCE", SES access such as ACCESS_KEY, SECRET_KEY, REGION
    ...
    ```yaml
      ses:
        image: getyoutrack/ses:v0.6.295
        container_name: ses
        ports:
          - 3335:3335
        environment:
...
Glint audio and video calls are created on top of LiveKit insfrastructure. In order to use Love service in your self-hosted Glint, perform the following steps:
...
2. Add `love` container to the docker-compose.yaml
    ...
    ```yaml
      love:
        image: getyoutrack/love:v0.6.295
        container_name: love
        ports:
          - 8096:8096
        environment:
...
You can configure a Glint instance to authorize users (sign-in/sign-up) using an OpenID Connect identity provider (IdP).

### On the IdP side

* Create a new OpenID application.
  * Use `{glint_account_svc}/auth/openid/callback` as the sign-in redirect URI. The `glint_account_svc` is the hostname for the account service of the deployment, which should be accessible externally from the client/browser side. In the provided example setup, the account service runs on port 3000.
